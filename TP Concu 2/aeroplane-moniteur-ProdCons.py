...
class UsineAeroplane :
    def __init__(self, tailleMaxTamponsChaines=2):
        ...
        for chaine in self.chaines:
            self.moniteur[chaine] = MoniteurProdCons(Tampon_fifo(tailleMaxTamponsChaines))
...
def carlingue(usine):
    for i in range(nbAvionsPrevus):
        ...
        while tampon_est_plein(usine.moniteur["carlingue"].tampon):
            usine.moniteur["carlingue"].fifoProd.wait()
        moniteur_deposer(usine.moniteur["carlingue"])

def aile() :
    for i in range(nbAvionsPrevus*2):
        time.sleep(random.randint(0, 2))
        tprint( 'Une Aile est achevée ({})'.format(i+1))
        while tampon_est_plein(usine.moniteur["aile"].tampon):
            usine.moniteur["aile"].fifoProd.wait()
        moniteur_deposer(usine.moniteur["aile"])

def roue():
    for i in range(nbAvionsPrevus*3):
        time.sleep(random.randint(0, 2))
        tprint( 'Une roue est achevée ({})'.format(i+1))
        while tampon_est_plein(usine.moniteur["roue"].tampon):
            usine.moniteur["roue"].fifoProd.wait()
        moniteur_deposer(usine.moniteur["roue"])
...
def moteur():
    for i in range(nbAvionsPrevus*2):
        ...
        while tampon_est_plein(usine.moniteur["moteur"].tampon):
            usine.moniteur["moteur"].fifoProd.wait()
        moniteur_deposer(usine.moniteur["moteur"])
...
def carlingue1Ailes2():
    for i in range(nbAvionsPrevus):
        ...
        if tampon_nbElements(usine.moniteur["carlingue"].tampon) >= 1 and tampon_nbElements(usine.moniteur["aile"].tampon) >= 2:

            moniteur_retirer(usine.moniteur["carlingue"], "carlingue1Ailes2")
            moniteur_retirer(usine.moniteur["aile"], "carlingue1Ailes2")
            moniteur_retirer(usine.moniteur["aile"], "carlingue1Ailes2")

        time.sleep(random.randint(0, 3))
        tprint( 'Un assemblage 1 carlingue avec 2 ailes est achevé ({})'.format(i+1))

        moniteur_deposer(usine.moniteur["carlingue1Ailes2"], "carlingue1Ailes2")


def carlingue1Ailes2Roues3():
    for i in range(nbAvionsPrevus):

        if tampon_nbElements(usine.moniteur["carlingue1Ailes2"].tampon) >= 1 and tampon_nbElements(usine.moniteur["roue"].tampon) >= 3:

            moniteur_retirer(usine.moniteur["carlingue1Ailes2"], "carlingue1Ailes2Roues3")
            moniteur_retirer(usine.moniteur["roue"], "carlingue1Ailes2Roues3")
            moniteur_retirer(usine.moniteur["roue"], "carlingue1Ailes2Roues3")
            moniteur_retirer(usine.moniteur["roue"], "carlingue1Ailes2Roues3")
        ...
        moniteur_deposer(usine.moniteur["carlingue1Ailes2Roues3"], "carlingue1Ailes2Roues3")


def aeroplane():
    for i in range(nbAvionsPrevus):

        if tampon_nbElements(usine.moniteur["carlingue1Ailes2Roues3"].tampon) >= 1 and tampon_nbElements(usine.moniteur["moteur"].tampon) >= 1:

            moniteur_retirer(usine.moniteur["carlingue1Ailes2Roues3"], "aeroplane")
            moniteur_retirer(usine.moniteur["moteur"], "aeroplane")

        time.sleep(random.randint(0, 3))
        tprint( 'Un aeroplane est achevé ({})'.format(i+1))

        moniteur_deposer(usine.moniteur["aeroplane"], "aeroplane")
