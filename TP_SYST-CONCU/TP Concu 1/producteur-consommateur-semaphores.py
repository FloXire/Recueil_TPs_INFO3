...
# Sémaphores
Slib = threading.Semaphore(tailleMaxTampon)
Socc = threading.Semaphore(0)
...
def producteur(nom)  :
    ...
    # prise d'un jeton dans le semaphore libre
    tprint('{nom} : Demande P(Slib)'.format(nom=nom))
    Slib.acquire()
    tprint('{nom} : Obtient P(Slib)'.format(nom=nom))

    # prise d'un jeton dans le semaphore em
    tprint('{nom} : Demande P(Sem)'.format(nom=nom))
    Sem.acquire()
    tprint('{nom} : Obtient P(Sem)'.format(nom=nom))
    ...
    tprint('{nom} : V(Socc)'.format(nom=nom))
    Socc.release()

    # remise du jeton dans le semaphore em
    tprint('{nom} : V(Sem)'.format(nom=nom))
    Sem.release()
...
def consommateur(nom)  :
    ...
    # prise d'un jeton dans le semaphore occupé
    tprint('{nom} : Demande P(Socc)'.format(nom=nom))
    Socc.acquire()
    tprint('{nom} : Obtient P(Socc)'.format(nom=nom))
    ...
    tprint('{nom} : V(Slib)'.format(nom=nom))
    Slib.release()
